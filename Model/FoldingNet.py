...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.graph_layer1 = GraphLayer(in_channel=128, out_channel=512, k=16)
        self.graph_layer2 = GraphLayer(in_channel=512, out_channel=4096, k=16)

        self.conv4 = nn.Conv1d(4096, 2048, 1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channel=2048):
        ...
        xx = np.linspace(-0.3, 0.3, 128, dtype=np.float32)
        yy = np.linspace(-0.3, 0.3, 160, dtype=np.float32)
        ...
        self.fold1 = FoldingLayer(in_channel + 2, [2048, 1024, 512, 3])
        self.fold2 = FoldingLayer(in_channel + 3, [2048, 1024, 512, 3])
...
class Loss(nn.Module):
    ...
    def forward(self, fine, gt, alpha, eps=0.005, iters=50):  # 0.005, 50 for training  0.002, 10000 is testing
        gt = gt.transpose(2, 1)
        fine = fine.transpose(2, 1)
